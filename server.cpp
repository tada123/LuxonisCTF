...
static const size_t CLIENT_BUFSZ = 16384;

uint64_t playerIdCounter = 16;
...
//    String id; //Long unique id (Currently set by client according to exam)
    ...
    bool guessing; //Guessing, or providing word
    char providedWord[1024];
...
void ClientInfo::setupConnectedClient(){
...
ssize_t findUserByName(const char* name){
    for(ssize_t i = 0; i < usersCount; i++){
        char* u = users[i];
        if(strcmp(u, name) == 0){
            return i;
        }
...
int main(int argc, char** argv){
    if((argc < 3) || (strcmp(argv[1], "--help") == 0)){
        printf("%s\n", "Usage:\nluxonis_game [type] [port||UDS file path]\nTypes:\n    TCP | Network accessable socket (over TCP protocol)\n    UDS | Unix Domain Socket");
        return 1;
    }
    
    signal(SIGPIPE, SIG_IGN); //Don't crash, if some client disconnects during I/O (Hang-ups are handled by epoll loop, so the fd will be closed in next epoll iteration)
    ...
    if(!f){
        fprintf(stderr, "%s", "ERROR: Cannot load `registeredUsers.txt` file (contains username=password list)\n");
        return 5;
    }
    usersAlloc = 1024;
    users = (char**)malloc(1024 * sizeof(char*));
    passwords = (char**)malloc(1024 * sizeof(char*));
    char buf[1024];
    size_t lineI = 0;
    while(fgets(buf, sizeof(buf), f)){
        if(usersCount >= usersAlloc){
            if(!(users = (char**)realloc(users, (usersAlloc *= 2) * sizeof(char*)))){
                perror("realloc");
                return 8;
            }
            if(!(passwords = (char**)realloc(users, (usersAlloc) * sizeof(char*)))){
                perror("realloc");
                return 8;
            }
        }
        char* eq = strchr(buf, '|');
        if(!eq){
            fprintf(stderr, "WARNING: '|' char expected on line %d\n", lineI);
            continue;
        }
        users[usersCount] = (char*)malloc(eq - buf + 1);
        memcpy(users[usersCount], buf, eq - buf);
        users[usersCount][eq - buf] = '\0';
        ++eq; //Skip '=' char
        char* nl = strchr(eq, '\n');
        if(nl){
            *nl = '\0'; //Don't include newline char in password
        }
        
        size_t passlen = strlen(eq) + 1 /*Include NUL char*/;
        passwords[usersCount] = (char*)malloc(passlen);
        memcpy(passwords[usersCount], eq, passlen);
        lineI++;
        usersCount++;
    }
    ...
    if(fam == AF_INET){
        ...
        addr.getInet().sin_port = htons(atoi(argv[2]));
        addr.getInet().sin_addr.s_addr = htonl(INADDR_ANY);
    }else{
        addr.getUnix().sun_family = fam;
        unlink(argv[2]);
        strcpy(addr.getUnix().sun_path, argv[2]);
    }
    if(bind(sock, addr.getGenericP(), (fam == AF_UNIX) ? sizeof(sockaddr_un) : sizeof(sockaddr_in)) != 0){
        perror("bind");
        return 10;
    }
    ...
    epoll_event ev;
    ev.events = EPOLLIN | EPOLLOUT | EPOLLHUP | EPOLLRDHUP | EPOLLERR |     EPOLLET /*Trigger only once per a real event (Don't trigger, if we just didn't write the fd yet )*/;
            ...
            if(currentEvent->data.u64 == -1){
                ...
                if(evflags & EPOLLIN){
                    while(1){
                        ...
                        ssize_t idx = findAvailableClientIndex();
                        if(idx < 0){
...
//                            currentSize += sizeof(ErrorCode);
                        ...
                        c->reset();
                        c->setupConnectedClient(); //Reset for new connection;
                        c->fd = a;
                    
                        char buf[1024];
                        *(uint64_t*) buf = c->id;
                        strcpy(buf + sizeof(uint64_t), welcomeMessage);
                        //Send initial handshake
                        client_writeData(c, msgtConnectionHandshake, (const uint8_t*) buf, sizeof(uint64_t) + sizeof(welcomeMessage));
...
//                        client_writeData(client);
                ...
                if(evflags & (EPOLLHUP | EPOLLRDHUP)){
                    if(!c->isPairingAvailable()){
                        //Pairedpasswords
                        client_writeData(&clients[c->paired], msgtGaveUp, NULL, 0);
                    }
                    client_close(c);
                        ...
                        if(c->authorized){
                            switch(reqType){
                                case garListOpponents: {
                                    ...
                                    uint8_t* buf = (uint8_t*)client_prepareWriteData(c, msgtOpponentsList, nassigned * (sizeof(uint64_t) + 1));
                                    if(!buf){
                                        client_writeErrorNow(c, ecLimitReached);
                                        continue;
                                    }
                                    for(size_t o = 0; o < MAX_CLIENTS; o++){
                                    ...
                                    c->outpos += nassigned * (sizeof(uint64_t) + 1);
                                ...
                                case garChooseOpponent: {
                                    ...
                                    psize_t crClientIdx = currentEvent->data.u64;
                                    ssize_t idx = findClientById(*(uint64_t*) msgPayload);
                                    if(idx < 0){
                                        client_writeError(c, ecInvalidOpponentId);
                                        continue;
                                    }
                                    if(c->isPairingAvailable() && clients[idx].isPairingAvailable()){
                                        c->paired = idx;
                                        c->guessing = true;
                                        clients[idx].paired = currentEvent->data.u64;
                                        clients[idx].guessing = false;
                                        
                                        uint8_t buf[sizeof(psize_t) + 1];
                                        *(psize_t*) buf = clients[idx].id;
                                        *(buf + sizeof(psize_t)) = 1; //You chose opponent, thus guessing
                                        client_writeData(c, msgtOpponentChosen, buf, sizeof(psize_t) + 1); 
                                        *(psize_t*) buf = c->id;
                                        *(buf + sizeof(psize_t)) = 0; //You were chosen, you write a secret word
                                        client_writeData(&clients[idx], msgtOpponentChosen, buf, sizeof(psize_t) + 1);
                                    }else{
                                ...
                                case garProvideWord: {
                                    if(c->isPairingAvailable()){
                                        //Not paired yet
                                        client_writeError(c, ecNotPairedYet);
                                        continue;
                                    }
                                    if(c->guessing){
                                        client_writeError(c, ecAnotherPosition);
                                        continue;
                                    }
                                    size_t cpcnt = std::min(payloadLen, sizeof(c->providedWord)) - 1;
                                    memcpy(c->providedWord, msgPayload, cpcnt);
                                    c->providedWord[cpcnt] = '\0';
                                    break;
                                }
                                case garGuessWord: {
                                    if(c->isPairingAvailable()){
                                    ...
                                    const char* providedWord = clients[c->paired].providedWord;
                                    if(!(*providedWord)){
                                        client_writeData(c, msgtWordNotPreparedYet, NULL, 0);
                                    }else if(strncmp(providedWord, (const char*)msgPayload, std::min(sizeof(ClientInfo::providedWord), payloadLen)) == 0){
                                        //Guessed !!!
                                        client_writeData(c, msgtGuessed, NULL, 0);
                                        client_writeData(&clients[c->paired], msgtGuessed, NULL, 0);
                                    }else{
                                        client_writeData(&clients[c->paired], msgtUnsuccessfulAttempt, NULL, 0);
                                        client_writeData(c, msgtUnsuccessfulAttempt, NULL, 0);
                                    }
                                    break;
                                }
                                case garSendTextToOpponent: {
                                    client_writeData(&clients[c->paired], msgtMessageFromOpponent, msgPayload, payloadLen);
                                    break;
                                }
                            }
                        }else{
                            ...
                            //Yes, the input should be properly sanitized in production environment
                            ssize_t idx = findUserByName((char*)msgPayload);
                            if(idx < 0){
                                client_writeError(c, ecInvalidUsername);
                                continue;
                            }
                            char* password = (char*) (msgPayload + strlen((char*)msgPayload) + 1); //2 null terminated strings
                            if(strcmp(passwords[idx], (char*) password) != 0){
                                client_writeError(c, ecWrongPassword);
                                continue;
                            }
                            c->authorized = true; //Successful auth
                            client_writeData(c, msgtSuccessfulLogin, NULL, 0);
                        }
                        //Message available
                    }
                }
